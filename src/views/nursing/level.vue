<template>
  <div style="max-width: 1600px; margin: 0 auto; padding: 24px;">
    ...
    <!-- 护理级别卡片 -->
    <el-card shadow="hover" style="margin-bottom: 32px;">
      ...
      <div style="margin-bottom:20px;">
        ...
        <el-button type="primary" @click="获取护理级别列表">查询</el-button>
        <el-button type="success" @click="新增级别弹窗 = true">新增级别</el-button>
      </div>

      <el-table :data="护理级别列表" border stripe style="margin-bottom:20px;">
        ...
        <el-table-column label="操作" align="center" width="260">
          <template #default="scope">
            <el-button type="primary" plain size="small" @click="打开修改状态(scope.row)">
              修改状态
            </el-button>
            <el-button type="warning" plain size="small" @click="打开项目配置(scope.row)">
              护理项目配置
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-card shadow="hover">
      ...
      <div style="display:flex; justify-content:space-between; margin-bottom:16px;">
        <div>
          <el-input v-model="搜索姓名" placeholder="按病人姓名查询"
                    style="width:260px;margin-right:10px;" />
          <el-button type="primary" @click="搜索病人">查询</el-button>
        </div>
        <el-button type="success" @click="新增病人弹窗=true">新增病人</el-button>
      </div>

      <el-table :data="分页病人列表" border stripe>
        <el-table-column label="编号" prop="id" align="center" />
        ...
        <el-table-column label="操作" align="center" width="140">
          <template #default="scope">
            <el-button type="primary" @click="打开日常护理(scope.row)">
              日常护理
            </el-button>
          </template>
        </el-table-column>
...
<div style="text-align: right; margin-top: 15px;">
  ...
  >
    ...
    <!-- 新增级别 -->
    <el-dialog v-model="新增级别弹窗" title="新增护理级别" center>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="保存新增级别">确认新增</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="修改级别弹窗" title="修改状态" center>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="保存修改级别">确认修改</el-button>
      </template>
    </el-dialog>

    <!-- 护理项目配置 -->
    <el-dialog v-model="项目配置弹窗" title="护理项目配置" width="900px" center>
      <div style="margin-bottom:10px; font-weight:bold;">当前级别：{{ 当前级别?.name }}</div>
      <div style="display: flex;gap:20px;">
        <div style="flex:1;">
          <h3>已配置护理项目</h3>
          <el-table :data="已配置项目列表" border height="400">
            <el-table-column label="项目名" prop="name" />
            <el-table-column label="操作">
              <template #default="scope">
                <el-button type="danger" link @click="移除项目(scope.row)">移除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
        <div style="flex:1;">
          <h3>可添加护理项目（启用）</h3>
          <el-table :data="可添加项目列表" border height="400">
            <el-table-column label="项目名" prop="name" />
            <el-table-column label="操作">
              <template #default="scope">
                <el-button type="success" link @click="添加项目(scope.row)">添加</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <el-dialog v-model="新增病人弹窗" title="新增病人" center>
      <el-form :model="新增病人表单" label-width="80px">
        ...
        <el-form-item label="年龄"><el-input v-model="新增病人表单.age" /></el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="保存新增病人">保存</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="日常护理弹窗" title="日常护理管理" width="900px" center>
      <div v-if="当前病人">
        ...
        <el-card shadow="hover" style="margin-bottom:20px;">
          <div slot="header" style="font-weight:bold;">病人基本信息</div>
          <el-form :model="当前病人" label-width="80px" inline>
            <el-form-item label="姓名"><el-input v-model="当前病人.name" /></el-form-item>
            <el-form-item label="年龄"><el-input v-model="当前病人.age" /></el-form-item>
            <el-form-item label="性别">
          ...
          <div style="margin-top:10px;">
            <el-button type="primary" size="small" @click="保存病人信息">保存病人信息</el-button>
            <el-button type="danger" size="small" @click="确认删除病人">删除该病人</el-button>
          </div>
        </el-card>

        <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
          <div style="font-weight:bold; font-size:16px;">护理记录</div>
          <el-button type="success" size="small" @click="新增护理记录">新增护理记录</el-button>
        </div>

        <el-table :data="当前病人.护理记录" border stripe>
          <el-table-column label="护理项目" prop="itemName" />
          <el-table-column label="记录时间" prop="recordTime" />
          <el-table-column label="操作" width="160">
            <template #default="scope">
              <el-button type="info" size="mini" @click="编辑护理记录(scope.row)">编辑</el-button>
              <el-button type="danger" size="mini" @click="删除护理记录(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog v-model="护理记录表单弹窗" title="护理记录" center>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="保存护理记录">保存</el-button>
      </template>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    保存新增级别() {
      ...
      this.护理级别列表.push({ id: 新编号, name: this.新增级别表单.name, status: this.新增级别表单.status });
      this.新增级别弹窗 = false;
